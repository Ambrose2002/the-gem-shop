...
export function CartDataProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (body?.ok && Array.isArray(body.lines)) {
        setLines(body.lines.map((r: any) => ({ productId: r.product_id, quantity: r.quantity })));
      }
    })();
  ...
  async function add(productId: string, qty = 1) {
    setLines(prev => {
      const f = prev.find(l => l.productId === productId);
      if (f) return prev.map(l => l.productId === productId ? { ...l, quantity: Math.min(99, l.quantity + qty) } : l);
  ...
  async function setQty(productId: string, qty: number) {
    setLines(prev => {
      ...
      return prev.map(l => l.productId === productId ? { ...l, quantity: Math.min(99, qty) } : l);
    });
    if (userId) {
  ...
  return <Ctx.Provider value={{ lines, add, remove, setQty, totalQty }}>{children}</Ctx.Provider>;
}
